...
4. **API-surface audit** - open [`PARITY.md`](../../../PARITY.md) and confirm every row still
   ...
   `/sessions/:id/blobs/:blobId` route and its security rules: blobId regex, Content-Type
   allowlist, `nosniff`, generic 404, `Cache-Control: immutable`). A capability present in one
   language but missing or differently-behaved in the other is a parity break, even if naming is
   (correctly) idiomatic per language.
...
5. **New-case audit** - if this change added a new `parseEntry`/`parse_entry` block type, a new
   ...
   behaviour change with no corpus case is a gap conformance cannot see.
